...
public class TreeProblems {
  ...
  public static <T> void postOrder(Map<T, List<T>> tree, T root) {
    ...
    for (T child : tree.get(root)) postOrder(tree, child);
  ...
  */
  public static <T> T findRoot(Map<T, List<T>> tree) {
    ...
    for (List<T> list : tree.values()) children.addAll(list);
    for (T node : tree.keySet()) if (!children.contains(node)) return node;
    return null;
  }
  ...
  */
  public static <T> int maxDepth(Node<T> root) {
    if (root == null) return 0;
    int max = 1;
    for (Node<T> child : root.children) max = Math.max(max, maxDepth(child) + 1);
    return max;
  }
  ...
  */
  public static int maxDepth(Map<String, List<String>> tree) {
    if (tree == null) return 0;
    return maxDepth(tree, findRoot(tree));
  }

  private static int maxDepth(Map<String, List<String>> tree, String node) {
    if (tree.get(node).isEmpty()) return 1;
    int max = 1;
    for (String child : tree.get(node)) max = Math.max(max, maxDepth(tree, child) + 1);
    return max;
  }
}
